...
// errorEnumGDScript is the GDScript source for the PB_ERR enum used by all
// generated proto files to report serialization errors.
const errorEnumGDScript = `enum PB_ERR {
	NO_ERRORS = 0,
	VARINT_NOT_FOUND = -1,
...
// errorEnumStatement returns the PB_ERR enum as a RawStatement.
func errorEnumStatement() gdast.RawStatement {
	return gdast.RawStatement{Code: errorEnumGDScript}
}
...
// sectionHeaderStatement returns a banner-style comment used to separate
// top-level sections of the generated file.
func sectionHeaderStatement(title string) gdast.RawStatement {
	bar := "# ============================================================================"
	return gdast.RawStatement{Code: bar + "\n# " + title + "\n" + bar}
}

// constantsSectionStatement returns the combined "Constants" banner and the
// PROTO_VERSION declaration as a single RawStatement, matching the layout of
// the golden output where the constant immediately follows the banner with no
// blank line in between.
func constantsSectionStatement() gdast.RawStatement {
	bar := "# ============================================================================"
