...
//go:embed proto_core_utils_data.gd
var protoCoreUtilsRaw string

// GenerateProtoCoreUtilsRaw returns the verbatim GDScript source for the
// sibling `proto_core_utils.gd` file. CLI callers should write the result
// next to the wrapper output. The content is embedded from
// `proto_core_utils_data.gd` (a copy of the golden) so it cannot drift.
func GenerateProtoCoreUtilsRaw() string {
	return protoCoreUtilsRaw
}
