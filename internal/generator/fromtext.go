...
func fromTextFloatBody(f *ast.Field, oneofGroup string) string {
	...
	b.WriteString("\t\t\t\t\t\t\"inf\":\n")
	b.WriteString("\t\t\t\t\t\t\tfloat_result = ProtoCoreUtils.NumberParseResult.from_float(INF, int(id_result[\"pos\"]))\n")
	b.WriteString("\t\t\t\t\t\t\"nan\":\n")
	b.WriteString("\t\t\t\t\t\t\tfloat_result = ProtoCoreUtils.NumberParseResult.from_float(NAN, int(id_result[\"pos\"]))\n")
	b.WriteString("\t\t\t\t\t\t_:\n")
